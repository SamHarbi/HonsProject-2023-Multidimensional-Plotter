...
One bottleneck though was indemnified during this development, which was to do with Automated testing. See \ref{automatedtests} for the details.
...
Test Driven development is an important aspect of the PXP process when developing software. But, during the creation of prototype 2 (See \ref{prototype2}) the student encountered a number of critical issues that contributed to the decision to not implement automated testing until further notice. Those issues were:

\begin{itemize}
    \item Graphics Development was found to be difficult to write tests for due to it's visual nature. This on it's own was not a problem as a solution using WatchUI was identified (See \ref{testframe}). What ended up being complex though was how rapidly the visuals changed during development, this made making tests useless as they could not be reused since they depended on comparing what the application rendered to reference images. This was thought to be most likely the result of the project being in it's early stages though. But the project never really reached a stage where this was feasible, almost every new feature would have required tests to be remade.
    \item Minimal UI to test, Automated testing on the web is particularly well suited to testing UI. Unfortunately, there wasn't any UI to test. This was once again thought to be the result of the project still being in it's early phase. But the project for the majority of development time had very minimal UI. Only in the last few sprints was there enough UI that manual testing couldn't be done in a few seconds. But, by that time the Student considered it too late to introduce automated testing.
\end{itemize}

The combination of the two above points made the time investment to setup tests not worth it over manually testing. This was expected to change as further development continued past the prototyping stage but this only started happening in the last few sprints. By then, there was not enough time to properly integrate automated testing as the focus was more on completing as many user stories as would be feasible.
